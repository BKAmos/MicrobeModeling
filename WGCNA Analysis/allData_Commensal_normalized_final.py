...
sample_meta = lotusCircRNA_GeneSelection[['New_rep','Treatment', 'Tissue', 'Round']] # remove round if seperating data by round
...
lotusCircRNAFiltered = lotusCircRNA_GeneSelection.drop_duplicates(subset =['circRNA_ID'], keep='first')
...
lotusCircRNAFiltered = lotusCircRNAFiltered.drop(columns=['Unnamed: 0',             'CIRI2_gene_id2', 'CIRI2_product2',
       ...
       'Round', 'Total_Count', 'CIRI2_RPK', 'CLEAR_RPK', 'RPK',
       'RPK_Sum_Sample', 'perMillScale', 'TPM'])
...
allGeneExp = PyWGCNA.WGCNA(name = 'All data Mean Filtered and TPM Normalized', species='bacteria', geneExpPath='gene_exp.csv', outputPath='outpath', save=True)
...
# %%
moduleOfInterest = ['gainsboro','lightgrey','white','brown','firebrick','darkgrey','dimgrey','whitesmoke','indianred','lightcoral']
